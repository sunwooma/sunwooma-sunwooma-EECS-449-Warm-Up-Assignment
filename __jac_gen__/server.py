...
class FeedbackChat(Chat, _Jac.Node):
    ...
    def respond(self, _jac_here_: infer) -> None:
        ...
        def collect_feedback(message: str, chat_history: list[dict]) -> str:
            return _Jac.with_llm(file_loc=__file__, model=llm, model_params={}, scope='server(Module).FeedbackChat(node).respond(Ability).collect_feedback(Ability)', incl_info=[], excl_info=[], inputs=[('current message', str, 'message', message), ('chat history', list[dict], 'chat_history', chat_history)], outputs=('response', 'str'), action='Collect user feedback and store it', _globals=globals(), _locals=locals())
        _jac_here_.response = collect_feedback(_jac_here_.message, _jac_here_.chat_history, agent_role='You are a feedback-collecting assistant. Please ask the user for their feedback about this interaction.')
